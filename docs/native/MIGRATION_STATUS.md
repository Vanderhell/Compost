...
| Opaque native ABI | DIFFERENTIAL_PASS | Python reference boundary | ABI v3 `compost_create/destroy/snapshot/digest/state_digest/select_partition/plan_division/partition/try_divide`, external-gut enqueue/process, and read-only conservation validation; opaque Python adapter validates the returned version and exposes failures without fallback |
| Python backend selector/loader | PORTING | Existing Python runtime | Explicit `python`/`native` handles, shared lifecycle-config forwarding, native step/snapshot/partition/try-divide adapters, and `checkpoint --backend` CLI mode; full lifecycle acceptance remains pending |
