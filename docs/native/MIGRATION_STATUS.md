...
| Sandbox `live_step` orchestration | PORTING | `AutonomousOrganism.live_step` | Explicit validated `NativeAction` plans, single-organism replay, and population `replay_action_traces` cover external-gut, queued-gut, corpse-credit, bounded metabolic-progress accounting, consolidation/maintenance/age with activity-debt settlement, starvation/death lifecycle replay, and retained numeric parent/child handles; filesystem/environment selection, sandbox identity/corpse handling, and complete event ordering remain Python-owned and are documented in `SANDBOX_TRANSITION_BOUNDARY.md` |
