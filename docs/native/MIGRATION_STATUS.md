...
| Sandbox `live_step` orchestration | PORTING | `AutonomousOrganism.live_step` | Explicit validated `NativeAction` plans and optional replayable action traces cover external-gut, queued-gut, corpse-credit, bounded metabolic-progress accounting, consolidation/maintenance/age, and a transient parent/child division checkpoint; filesystem/environment selection, child registration/corpse handling, death, and complete event ordering remain Python-owned and are documented in `SANDBOX_TRANSITION_BOUNDARY.md` |
