...
| Opaque native ABI | DIFFERENTIAL_PASS | Python reference boundary | ABI v3 `compost_create/destroy/snapshot/digest/state_digest/select_partition/plan_division/partition`, external-gut enqueue/process, and read-only conservation validation; opaque Python adapter validates the returned version and exposes failures without fallback |
