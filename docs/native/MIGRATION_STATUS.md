...
| Resorption/external FIFO accounting | DIFFERENTIAL_PASS | `AutonomousOrganism.enqueue_external_material`, `process_gut` | Native ABI v4 owns bounded payload/nutrition chunks, partial FIFO processing, resorption accounting, lifecycle bookkeeping, and Python adapter tests; full sandbox differential integration remains pending |
...
| Opaque native ABI | DIFFERENTIAL_PASS | Python reference boundary | ABI v4 `compost_create/destroy/snapshot/restore_snapshot/digest/state_digest/select_partition/select_local_reproduction/plan_division/partition/try_divide/try_local_reproduction/weaken_weakest`, pure step, explicit lifecycle-step settlement, external-gut enqueue/process, corpse-energy credit, read-only conservation validation, ID-ordered population state digests, explicit action traces, lifecycle bookkeeping fields, and dead-snapshot handle transfer; native snapshot restore is transactional and conservation-validated; opaque Python adapter validates the returned version and exposes failures without fallback; population due-lifecycle and action-trace epochs now have exact cross-handle rollback |
| Python backend selector/loader | PORTING | Existing Python runtime | Explicit `python`/`native`/`native-population` handles, top-level `NativePopulationBackend` export, shared lifecycle-config forwarding, native step/snapshot/partition/try-divide/step-plus-division/due-lifecycle adapters, ID-ordered orchestration, atomic single-handle action traces and multi-handle environment/action/due epochs with opaque rollback, atomic direct reproduction/partition child registration with snapshot-failure rollback, retained explicit-division child handles with collision preflight, corpse request/transfer operations, and `checkpoint --backend` CLI mode; full sandbox lifecycle acceptance remains pending |
...
| Public API robustness tests | DIFFERENTIAL_PASS | C API failure contract | Null, NaN, transactional failure, caller-output preservation, allocator lifetime/failure, integer extrema, idempotent destroy, deterministic 10,000-case fuzzing, and fixed-width ABI audit coverage; hostile serialized-state coverage remains pending |
...
| Metabolic schedule arithmetic | DIFFERENTIAL_PASS | `AutonomousOrganism.metabolic_work_threshold` | Native bounded threshold/quotient/remainder helper matches Python max/divmod cases, including uint64 boundaries; host supplies accepted work while native computes due counts |
| Native metabolic progress accumulator | DIFFERENTIAL_PASS | `AutonomousOrganism.metabolic_progress`, `metabolic_steps` | Opaque ABI v4 context owns explicit transactional progress accounting; the population replay boundary now accumulates progress and executes all resulting due lifecycle checkpoints in one native transaction, with early dead-state stop and per-epoch Python comparison |
| Reproduction assessment | DIFFERENTIAL_PASS | `biology_rules.reproduction_allowed` | Pure eligibility/score differential cases pass; weakest-member and boundary-partition candidate policies are both ported and exercised by the native sandbox adapter |
| Local reproduction component selection | DIFFERENTIAL_PASS | `AutonomousOrganism._reproduce_conservatively` | Read-only C17 weakest-member/component selector and explicit `try_local_reproduction` transaction match the Python reference on deterministic graph cases, including maximum isolated-component capacity; the native boundary-partition `try_divide` policy covers the historical `_divide_locally` branch |
...
| Sandbox `live_step` orchestration | PORTING | `AutonomousOrganism.live_step` | Reusable `NativeSandboxReplay` transactionally imports the C-representable subset of initial evolved organisms, invokes host-built `NativeAction` plans, owns deterministic numeric parent/child handles, wraps complete native epochs in exact opaque-state rollback including interrupted corpse-handle recreation, emits and natively replays host-approved territory reclaim mutations, runs native local-reproduction and boundary-partition policies for division traces, and now executes `METABOLIC_PROGRESS` together with all resulting due lifecycle steps through one transactional native population boundary; it captures post-mutation Python states at action boundaries and compares every shared native/Python field with first-field diagnostics, returns epoch snapshots, emits one-shot `store_corpse` requests, and transfers dead snapshots; external-gut, queued-gut, corpse-credit, territory, material flow, full activity-ledger settlement, ordered gut state, metabolic progress/step count, generation, activated receptors, consolidation/maintenance/age, and starvation/death replay are covered; the remaining gap is native-first planning from Python-only navigation/environment state, while filesystem/environment selection, sandbox identity/corpse persistence, territory release, and complete event ordering remain intentionally Python-owned and are documented in `SANDBOX_TRANSITION_BOUNDARY.md` |

| Selected structural partition | DIFFERENTIAL_PASS | `AutonomousOrganism._commit_skeleton_partition` | Selector, viability plan, local reproduction transaction, native boundary-partition transaction, and conservation checks pass native GCC/MSVC tests plus bounded Python/sandbox mass/reserve/body differential coverage; broader lifecycle scheduling remains separate |
| Native parallel runtime | DEFERRED | Existing Python parallel runtime | See `PARALLEL_RUNTIME.md`; single-thread semantic parity is required first; CI now runs the native sandbox boundary suite on Linux and Windows before the complete Python suite |
