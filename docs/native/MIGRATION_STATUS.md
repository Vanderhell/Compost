...
| Sandbox `live_step` orchestration | PORTING | `AutonomousOrganism.live_step` | Explicit native gut, corpse-credit, partition, and lifecycle slices exist; the filesystem/environment action plan and complete event ordering remain Python-owned and are documented in `SANDBOX_TRANSITION_BOUNDARY.md` |
