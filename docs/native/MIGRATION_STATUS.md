...
| Sandbox `live_step` orchestration | PORTING | `AutonomousOrganism.live_step` | Explicit validated `NativeAction` plans and optional replayable action traces cover external-gut, queued-gut, corpse-credit, and one physical-food lifecycle checkpoint; filesystem/environment selection, metabolic scheduling, and complete event ordering remain Python-owned and are documented in `SANDBOX_TRANSITION_BOUNDARY.md` |
