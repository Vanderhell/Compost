...
| Opaque native ABI | DIFFERENTIAL_PASS | Python reference boundary | ABI v3 `compost_create/destroy/snapshot/digest/state_digest/select_partition/plan_division/partition/try_divide/weaken_weakest`, pure step, explicit lifecycle-step settlement, external-gut enqueue/process, read-only conservation validation, and ID-ordered population state digests; opaque Python adapter validates the returned version and exposes failures without fallback |
| Python backend selector/loader | PORTING | Existing Python runtime | Explicit `python`/`native`/`native-population` handles, shared lifecycle-config forwarding, native step/snapshot/partition/try-divide/step-plus-division adapters, ID-ordered orchestration, retained explicit-division child handles with collision preflight, and `checkpoint --backend` CLI mode; full sandbox lifecycle acceptance remains pending |
...
| Sandbox `live_step` orchestration | PORTING | `AutonomousOrganism.live_step` | Explicit validated `NativeAction` plans, single-organism replay, population `replay_action_traces`, retained numeric parent/child handles, and one-shot `store_corpse` requests on ALIVE-to-DEAD transitions cover external-gut, queued-gut, corpse-credit, bounded metabolic-progress accounting, consolidation/maintenance/age with activity-debt settlement, and starvation/death replay; filesystem/environment selection, sandbox identity/corpse persistence, territory release, and complete event ordering remain Python-owned and are documented in `SANDBOX_TRANSITION_BOUNDARY.md` |
