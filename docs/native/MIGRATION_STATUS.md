...
| Python backend selector/loader | PORTING | Existing Python runtime | Explicit `python`/`native` handles, shared lifecycle-config forwarding, native step/snapshot/partition/try-divide/step-plus-division adapters, and `checkpoint --backend` CLI mode; full lifecycle acceptance remains pending |
