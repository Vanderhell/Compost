...
| Native metabolic progress accumulator | DIFFERENTIAL_PASS | `AutonomousOrganism.metabolic_progress`, `metabolic_steps` | Opaque ABI v4 context owns explicit transactional progress accounting; due work is reported without pre-crediting `metabolic_steps`, and only successful non-dead lifecycle checkpoints increment completed steps; batch execution matches repeated native checkpoints, while automatic Python event ordering remains host-owned |
