...
| Sandbox `live_step` orchestration | PORTING | `AutonomousOrganism.live_step` | Explicit validated `NativeAction` plans, single-organism replay, population `replay_action_traces`, retained numeric parent/child handles, one-shot `store_corpse` requests, and dead-snapshot transfer/handle cleanup cover external-gut, queued-gut, corpse-credit, bounded metabolic-progress accounting, consolidation/maintenance/age with activity-debt settlement, and starvation/death replay; filesystem/environment selection, sandbox identity/corpse persistence, territory release, and complete event ordering remain Python-owned and are documented in `SANDBOX_TRANSITION_BOUNDARY.md` |
