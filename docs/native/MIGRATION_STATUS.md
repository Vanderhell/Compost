...
| Native metabolic progress accumulator | DIFFERENTIAL_PASS | `AutonomousOrganism.metabolic_progress`, `metabolic_steps` | Opaque ABI v4 context owns explicit transactional progress/step accounting, lifecycle bookkeeping, and an empty-input due-lifecycle batch endpoint; batch execution matches repeated native checkpoints, while automatic Python event ordering remains host-owned |
