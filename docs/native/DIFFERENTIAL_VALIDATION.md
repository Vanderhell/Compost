...
to Python fallback. The current bounded campaign contains seventy tests,
...
run is green (`70 passed`; the focused oracle/member-weakness validation run
is `73 passed` including the related Python tests), and the complete Python regression suite is green
(`270 passed, 3 subtests passed` in the latest ABI v4 run). The selected partition
