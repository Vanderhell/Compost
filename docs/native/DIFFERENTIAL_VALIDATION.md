...
to Python fallback. The current bounded campaign contains sixty-one tests,
...
run is green (`61 passed`; the focused oracle/member-weakness validation run
is `64 passed` including the related Python tests), and the complete Python regression suite is green
(`258 passed, 3 subtests passed` in the latest ABI v4 run). The selected partition
