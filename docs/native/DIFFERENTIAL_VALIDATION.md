...
to Python fallback. The current bounded campaign contains sixty-two tests,
...
run is green (`62 passed`; the focused oracle/member-weakness validation run
is `65 passed` including the related Python tests), and the complete Python regression suite is green
(`259 passed, 3 subtests passed` in the latest ABI v4 run). The selected partition
