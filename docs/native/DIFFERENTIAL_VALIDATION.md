...
to Python fallback. The current bounded campaign contains sixty-six tests,
...
run is green (`66 passed`; the focused oracle/member-weakness validation run
is `69 passed` including the related Python tests), and the complete Python regression suite is green
(`266 passed, 3 subtests passed` in the latest ABI v4 run). The selected partition
