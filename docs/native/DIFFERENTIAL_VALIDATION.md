...
to Python fallback. The current bounded campaign contains sixty tests,
...
run is green (`60 passed`; the focused oracle/member-weakness validation run
is `63 passed` including the related Python tests), and the complete Python regression suite is green
(`257 passed, 3 subtests passed` in the latest ABI v4 run). The selected partition
