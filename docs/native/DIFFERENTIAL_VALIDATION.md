...
to Python fallback. The current bounded campaign contains fifty-eight tests,
...
run is green (`58 passed`; the focused oracle/member-weakness validation run
is `61 passed` including the related Python tests), and the complete Python regression suite is green
(`252 passed, 3 subtests passed` in the latest ABI v4 run). The selected partition
...
with epoch-wide ID/count preflight and per-population conservation validation.
