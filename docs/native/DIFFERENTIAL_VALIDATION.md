...
to Python fallback. The current bounded campaign contains fifty-nine tests,
...
run is green (`59 passed`; the focused oracle/member-weakness validation run
is `62 passed` including the related Python tests), and the complete Python regression suite is green
