...
Python tests:
`213 passed, 3 subtests passed` in the latest complete local run with the
native DLL configured (83.43s).

Native tests:
`7/7` CTest tests passed in the current MSVC 19.42 Release build with
warnings-as-errors enabled (9.98s); the same suite passed under GCC/WSL.

Differential cases:
`22` Python/native differential tests pass, including bounded pure-rule,
