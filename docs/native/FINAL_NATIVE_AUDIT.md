...
Python tests:
`271 passed, 3 subtests passed` in the latest complete local run with the
ABI v4 native DLL configured (106.00s).
...
Differential cases:
`71` Python/native differential tests pass, including bounded pure-rule,
...
plus strict non-uint64 population-ID and constructor validation. A multi-epoch
sandbox trace campaign also replays
