...
Python tests:
`257 passed, 3 subtests passed` in the latest complete local run with the
ABI v4 native DLL configured (103.74s).
...
Differential cases:
`60` Python/native differential tests pass, including bounded pure-rule,
...
The latest ABI v4 MSVC Debug DLL differential run completed in 26.10s; the
