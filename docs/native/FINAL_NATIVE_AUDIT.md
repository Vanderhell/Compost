...
Python tests:
`258 passed, 3 subtests passed` in the latest complete local run with the
ABI v4 native DLL configured (140.87s).
...
Differential cases:
`61` Python/native differential tests pass, including bounded pure-rule,
...
The latest ABI v4 MSVC Debug DLL differential run completed in 31.49s; the
